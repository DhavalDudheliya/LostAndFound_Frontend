...
function FoundItemsCoordinator() {
  ...
  useEffect(() => {
    axios
      .all([axios.get(`/coordinator/getFoundItems`)]) // Here i use same route that used in Coordinator
      .then(
        axios.spread((res1) => {
  ...
  const handleSearch = async () => {
    ...
    axios
      .all([
        axios.get(
          `/coordinator/getFoundItemsBySearch?q=${query}` // Here i use same route that used in Coordinator
        ),
      ])
      .then(
        axios.spread((res1) => {
